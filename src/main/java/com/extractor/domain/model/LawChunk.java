...
@Slf4j
@ToString
public class LawChunk extends ChunkOld implements ChunkProcessor {
     ...
     * @return 법령 청크 목록
     */
    private static List<ChunkOld> chunking(LawChunk chunk) {
        ...
        if (nextDepth >= chunk.depthSize) {
            ...
            if (tokenSize > chunk.maxTokenSize) {
                ...
                if (chunk.lawContents.size() > 1) {
                    ...
                    chunksByToken.addAll(chunking(new LawChunk(chunk, chunk.lawContents.subList(0, mid))));
                    chunksByToken.addAll(chunking(new LawChunk(chunk, chunk.lawContents.subList(mid, chunk.lawContents.size()))));
                }
                // content 하나인 경우
        ...
        for (int contentIndex = 0; contentIndex < chunk.lawContents.size(); contentIndex++) {
            ...
            for (int prefixIndex = 0; prefixIndex < patternVo.getPrefixes().size(); prefixIndex++) {
                ...
                if (prefix.getPrefix().equals(lawContent.getCategoryCode())) {
                    // 재귀
                    chunks.addAll(chunking(new LawChunk(chunk, chunk.lawContents.subList(Math.max(head, 0), contentIndex))));
        ...
        chunks.addAll(chunking(new LawChunk(chunk, chunk.lawContents.subList(Math.max(head, 0), chunk.lawContents.size()))));
