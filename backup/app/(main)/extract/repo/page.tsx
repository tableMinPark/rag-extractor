...
export default function UrlChunkPage() {
  // ###################################################
  // 상태 정의 (State)
  // ###################################################
  ...
  const [chunkConfig, setChunkConfig] = useState<ChunkConfig>({
    ...
    regexPatterns: {
      depth1: [{ prefix: '^"title":', isTitle: true }],
      depth2: [],
      depth3: [],
    },
    stopPatterns: [],
  ...
  // 소스별 접기/펼치기 상태 (true: 접힘, false: 펼쳐짐)
  ...
  const isRunEnabled = () => {
    ...
    const hasValidUrl = targetUrls.some((url) => url.trim().length > 0)
  ...
  const handleExtract = async () => {
    ...
    try {
      await measureRequest(async () => {
        ...
        // [Mock Data Logic]
        const validUrls = targetUrls.filter((u) => u.trim() !== '')
        const mockGroups: SourceGroup[] = validUrls.map((url, urlIdx) => {
          return {
            sourceUrl: url,
            chunks: Array.from({
              length: 2 + Math.floor(Math.random() * 3),
            }).map((_, i) => ({
              title: `Source ${urlIdx + 1} - Section ${i + 1}`,
              subTitle: `Category: API_DATA`,
              thirdTitle: `ID: ${urlIdx * 100 + i}`,
              content: `Content from ${url}.\nThis is chunk #${i + 1}. 청킹 설정(${chunkConfig.chunkingType})에 따라 분할된 텍스트입니다.`,
              subContent: `Fetched At: ${new Date().toLocaleTimeString()}\nOrigin: ${url}`,
            })),
          }
        })

        // [수정] 모든 그룹을 기본적으로 '접힘(true)' 상태로 설정
        const initialCollapsedState: Record<string, boolean> = {}
        mockGroups.forEach((g) => {
          initialCollapsedState[g.sourceUrl] = true // 기본값: 닫힘
        })
  ...
  const toggleSourceCollapse = (sourceUrl: string) => {
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              원격지 JSON/데이터 소스를 호출하여 청킹 결과를 테스트합니다.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="scrollbar-thin scrollbar-thumb-gray-200 flex w-[450px] flex-col gap-6 overflow-y-auto pr-2">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="flex items-center gap-2 text-sm font-bold text-gray-700">
                <LinkIcon className="text-primary h-4 w-4" /> 대상 URL 입력
              </h3>
              <button
                onClick={addUrl}
              ...
              {targetUrls.map((url, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-transparent text-gray-400 transition-colors hover:border-red-100 hover:bg-red-50 hover:text-red-500"
                  >
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-5">
              {/* 추출 타입 */}
              <div>
                <label className={labelClass}>변환 포맷</label>
                ...
                {extractedResults.map((group, groupIdx) => (
                  <div key={groupIdx} className="flex flex-col gap-3">
                    ...
                    <div
                      ...
                      onClick={() => toggleSourceCollapse(group.sourceUrl)}
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col overflow-hidden">
                          ...
                          <span className="text-[10px] text-gray-500">
                            {group.chunks.length} chunks generated
                          </span>
                        </div>
                      </div>
                      <div className="text-gray-400">
                        {collapsedSources[group.sourceUrl] ? (
