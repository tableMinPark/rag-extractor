...
export const ModalModifySource = ({
  ...
  const handleDownloadFile = (index: number) => {
    console.log(formData.fileResources[index])
  }
          ...
          <div className="flex flex-col gap-10">
            ...
            <section className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <label
                  className={`relative flex items-center gap-4 rounded-xl border-2 p-4 transition-all ${formData.sourceType === 'file' ? 'border-primary bg-primary/5 ring-primary/20 ring-1' : 'border-gray-100 bg-white'}`}
                >
                  <div
                ...
                <label
                  className={`relative flex items-center gap-4 rounded-xl border-2 p-4 transition-all ${formData.sourceType === 'repo' ? 'border-primary bg-primary/5 ring-primary/20 ring-1' : 'border-gray-100 bg-white'}`}
                >
                  <div
                    ...
                    {formData.fileResources.length > 0 && (
                      <div className="mb-2 flex items-end justify-between">
                        <span className="text-xs font-bold text-gray-600">
                          등록된 파일 ({formData.fileResources.length})
                        </span>
                      </div>
                    )}
                    {formData.fileResources.map((fileResource, index) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <div className="flex-1 overflow-hidden">
                          <p className="truncate text-xs font-bold text-gray-800">
                            {fileResource.originFileName}.{fileResource.ext}
                          </p>
                          <p className="text-[10px] text-gray-500">
                            {(fileResource.fileSize / 1024).toFixed(1)} KB
                          </p>
                        </div>
                        <button
