...
export default function SearchPage() {
  // ###################################################
  // [설정] 페이지 모드 설정 (Keyword vs Vector)
  // ###################################################
  // ※ 키워드 검색 페이지에서는 이 부분을 변경하세요.
  const PAGE_MODE: 'VECTOR' | 'KEYWORD' = 'VECTOR'
  ...
  const PageDesc =
    PAGE_MODE === 'VECTOR'
      ? '임베딩된 벡터 공간에서 의미론적 유사도가 높은 문서를 검색합니다.'
      : '형태소 분석을 통해 키워드가 정확히 일치하거나 포함된 문서를 검색합니다.'
  const MainIcon = PAGE_MODE === 'VECTOR' ? SearchCode : Type
  ...
  const handleSearch = async (newPage = 1) => {
    ...
    try {
      ...
      const mockData: SearchResultItem[] = Array.from({
        ...
        score:
          PAGE_MODE === 'VECTOR'
            ? Number((0.92 - i * 0.03).toFixed(4)) // 0.xx (Cosine Similarity)
            : Number((15.5 - i).toFixed(1)), // xx.x (BM25 Score)
        categoryCode: categories[i % categories.length].code,
        title: `개인정보 보호법 제${20 + i}조 (검색 테스트 문서 ${i + 1})`,
        subTitle: `제${i + 1}장 총칙 > 제${i * 2}절`,
        content: `검색어 "${query}"에 대한 ${PAGE_MODE} 결과입니다. \n이 법은 개인정보의 처리에 관한 사항을 정함으로써 개인의 자유와 권리를 보호하고, 나아가 개인의 존엄과 가치를 구현함을 목적으로 한다.`,
        subContent: `[시행 2024. 3. 15.] [법률 제19234호, 2023. 3. 14., 일부개정] \n담당부서: 개인정보보호위원회`,
        createDt: '2024-03-20',
      }))

      setResults(mockData)
      setTotalCount(45) // 전체 45개 가정
    } catch (error) {
      console.error(error)
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="flex h-full w-full flex-col p-6">
      ...
      <div className="mb-6 flex items-center gap-3">
        <MainIcon className="text-primary h-8 w-8" />
        <div>
          <h2 className="text-2xl font-bold text-gray-800">{PageTitle}</h2>
          <p className="mt-1 text-xs text-gray-500">{PageDesc}</p>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          <span className="text-xs text-gray-400">
            {PAGE_MODE === 'VECTOR'
              ? '문장의 의미가 유사한 내용을 검색합니다.'
              : '단어가 정확히 일치하는 문서를 찾습니다.'}
          </span>
        </div>
      </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="flex items-center px-2">
                {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
                  const pNum = i + 1 // 실제 로직에선 현재 페이지 기준 계산 필요
                  return (
                    <button
                      key={pNum}
