    ...
    <div className="flex h-full w-full flex-col p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">패시지 상세 정보 & 청크 목록</p>
        </div>
        <button
          onClick={() => router.back()}
          ...
          <div className="flex-1 overflow-auto p-8">
            <div className="mx-auto flex max-w-3xl flex-col gap-8">
              <div className="flex flex-col gap-2 border-b border-gray-100 pb-6">
                ...
                {passage.thirdTitle && (
                  <h3 className="text-md border-l-2 border-gray-200 pl-3 font-medium text-gray-600">{passage.thirdTitle}</h3>
                )}
        ...
        <div className="flex flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-6 py-4">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-700">{totalCounts}</span>
            </div>
            <button
              ...
              {chunkList.map((chunk) => (
                ...
                >
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-gray-500">#{chunk.chunkId}</span>
                    ...
                    <span className="text-[10px] font-medium text-gray-400">{chunk.compactContentTokenSize} Tokens</span>
                  </div>
                  <p className="line-clamp-3 text-sm leading-relaxed text-gray-600 group-hover:text-gray-900">{chunk.content}</p>
                  <div className="text-primary absolute right-4 bottom-4 -translate-x-2 opacity-0 transition-all group-hover:translate-x-0 group-hover:opacity-100">→</div>
                </Link>
              ))}
              {chunkList.length === 0 && (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 border-t border-gray-100 bg-gray-50 px-6 py-3">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page === 1}
                className="rounded border border-gray-300 bg-white px-3 py-1 text-xs font-medium text-gray-600 disabled:opacity-50"
              >
                이전
              </button>
