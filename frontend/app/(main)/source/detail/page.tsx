...
// [컴포넌트] 배칭 로딩 모달
// ###################################################
const BatchLoadingModal = () => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-all">
      <div className="animate-in fade-in zoom-in-95 flex flex-col items-center justify-center gap-5 rounded-2xl bg-white p-10 shadow-2xl duration-300">
...
function SourceDetailContent() {
  ...
  // UI 상태
  const [isBatch, setIsBatch] = useState(false) // 배치 작업 진행 중 여부
  ...
  // [수정됨] 청킹 배치 실행 핸들러
  const handleChunkBatch = async (sourceId: number) => {
    if (
      !confirm(
        '청킹 배치를 시작하시겠습니까?\n문서 크기에 따라 시간이 소요될 수 있습니다.',
      )
    ) {
      return
    }

    setIsBatch(true) // 1. 모달 띄우기

    try {
      const response = await chunkBatchApi(sourceId)
      console.log(`📡 배치 결과: ${response.message}`)
  ...
  function updateStatusBadge(updateState: string) {
    ...
    const color = updateStatusColors[status] || 'gray'
    const name = updateStatusNames[status] || '알수없음'

    return (
      <span
        className={`inline-flex items-center rounded-full bg-${color}-50 px-2 py-0.5 text-xs font-medium text-${color}-700 ring-1 ring-${color}-700/10 ring-inset`}
      >
        {name}
      </span>
    )
        ...
        <div className="mb-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {getRole() === 'ROLE_ADMIN' && (
                <button
                  onClick={() => handleChunkBatch(sourceId)}
                  disabled={isBatch} // 작업 중 클릭 방지
                ...
                >
                  <RefreshCw
                    className={`h-4 w-4 ${isBatch ? 'animate-spin' : ''}`}
                  />
                  <span>{isBatch ? '작업 중...' : '청킹배치'}</span>
                </button>
              )}
